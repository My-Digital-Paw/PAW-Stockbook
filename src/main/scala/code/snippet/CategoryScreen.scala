...
object CategoryScreen extends LiftScreen {
  object category extends ScreenVar(ProductCategory.find(asLong(S.param("id")) openOr 0L) openOr ProductCategory.create)
  ...
  override def localSetup() {
    ProductCategory.findAll.foreach(ctg => parents.get.put(ctg.id, ctg.name.is))
  }
